...
public class Button_FormatedDollInfo : MonoBehaviour
{
    ...
    void Refresh() {
        ...
        if (maxhp - model.GetComponent<FinalState>().hp >= 1) {
            ...
            if (placed) 
                Button_heal.interactable = false;
            else 
                Button_heal.interactable = true;
            ...
            switch (model.GetComponent<OriginalState>().dollstate._class) {
                ...
                //남은 체력 비율
                hp_rate = Mathf.Max((float)model.GetComponent<FinalState>().hp, 0) / (float)maxhp;
                //수복 인력 계산
                cost_heal = Mathf.CeilToInt(type_c * (1.0f - hp_rate));
                //수복 부품 계산
                part_heal = Mathf.CeilToInt(type_p * (1.0f - hp_rate));
