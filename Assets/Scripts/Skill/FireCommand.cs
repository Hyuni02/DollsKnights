...
public class FireCommand : SkillBase
{
    ...
    public override void SkillActive() {
        ...
        for (int i = 0; i < InGameManager.instance.Spawned_Dolls.Count; i++) {
            ...
            buff_M1873.GetComponent<Buff>().Initialized(GetSkillName(), GetDuration(), GetComponent<SkillBase>().skill_icon, gameObject, InGameManager.instance.Spawned_Dolls[i], increase_dmg[level - 1], 0, 0, 0, 0, 0, 0, false, 0);
        }
    }
    public override void SkillDescribe() {
        base.SkillDescribe();

        skill_describe = skill_describe_form.Replace("_dmg", increase_dmg[level - 1].ToString());
    }
}
